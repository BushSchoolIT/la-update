...
new_rows = []
existing_emails = set(
    existing_students["Student Email Address"].astype(str).str.strip()
)
...
for index, row in bb_students.iterrows():
    ...
    if student_email not in existing_emails:
        match_index = bb_students.index[
            bb_students["Student Email Address"].astype(str).str.strip().str.lower()
            == student_email
        ][0]
        print(match_index)
        if student_id in existing_ids:
            updated_students.loc[
                updated_students["Student ID Number"] == student_id,
                "Student Email Address",
            ] = student_email
            ...
            for col in col_list:
                ...
                new_row["Password"] = "TEMP_PASSWORD"
                new_row["School Name"] = "BUSH SCHOOL"
...
if new_rows:
    ...
    additions["Do NOT Edit - Learning Ally Key"] = pd.to_numeric(
        additions["Do NOT Edit - Learning Ally Key"], errors="coerce"
    ).astype("Int64")
